...
All steps below are **done and merged** — markers reflect a 2026-06-01 re-check against the code on `main`.

### 1. Reshape types — ✅ done — [extension/src/lib/types.d.ts](../extension/src/lib/types.d.ts)
...
### 2. Helpers — ✅ done — [extension/src/lib/oidc.ts](../extension/src/lib/oidc.ts)
...
### 3. Service worker — ✅ done — [extension/src/background.ts](../extension/src/background.ts)
...
### 4. Sidebar — ✅ done — [extension/src/lib/auth.svelte.ts](../extension/src/lib/auth.svelte.ts)
...
### 5. Tests — ✅ done (33/33 green) — [extension/src/lib/tests/oidc.test.ts](../extension/src/lib/tests/oidc.test.ts)
...
### 6. Docs — ✅ done — [extension/CLAUDE.md](../extension/CLAUDE.md)
...
## Confirm before shipping — ✅ verified
...
- ✅ **JWT endpoint is on the Squarelet you point at.** If `MUCKROCK_ACCOUNTS_HOST` ever targets an older deployment, `/api/jwt/` will 404 and sign-in will fail outright. Confirmed present on the targeted deployment.

## Rollout

1. ✅ Landed — merged to `main` (the work shipped across `9-list-alerts` and successor branches, not a single PR).
2. ✅ Manual QA against dev Squarelet — verified:
